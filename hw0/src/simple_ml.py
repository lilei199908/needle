...
def softmax_loss(Z, y):
    ...
    softmax_loss = softmax[np.arange(y.shape[0]), y]
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for step in range(num_step):
        ...
        # Compute softmax
        safe_max = Z2.max(axis=1, keepdims=True)
        Z2_exp = np.exp(Z2 - safe_max)
        Z2_exp_sum = Z2_exp.sum(axis=1, keepdims=True)
        softmax = Z2_exp / Z2_exp_sum

        # Backward pass
        softmax[np.arange(y_batch.shape[0]), y_batch] -= 1
        grad_W2 = A1.T @ softmax / y_batch.shape[0]

        dA1 = softmax @ W2.T
        dZ1 = dA1 * (Z1 > 0)  # Gradient through ReLU
        grad_W1 = X_batch.T @ dZ1 / y_batch.shape[0]
